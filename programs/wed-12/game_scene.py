...
# Edited on 23 Nov 2016: Removed Deepcopy from the program
# Edited on 30 Nov 2016: Created a scoreboard. Created menu button when game over. added scoreboard functionality.
# Edited on 6 Dec 2016: added sounds and prevented spaceship from going off-screen
...
class GameScene(Scene):
    ...
    def update(self):
        ...
        if self.left_button_pressed == True and self.spaceship.position.x > 100:
            self.spaceship.run_action(Action.move_by(-1*self.speed_of_spaceship, 0.0, 0.1))
        if self.right_button_pressed == True and self.spaceship.position.x < self.size_of_screen_x - 100:
